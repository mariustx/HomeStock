...
export function AddItemModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      // Initialize sensible section open states
      if (itemToEdit) {
        const hasCustomPackaging =
          (itemToEdit.unit && itemToEdit.unit.toLowerCase() !== 'piece') ||
          Boolean(itemToEdit.purchase_package) ||
          (itemToEdit.units_per_package != null && itemToEdit.units_per_package > 1);
        const hasConsumptionData = Boolean(itemToEdit.opened_at);
        const hasOtherData = Boolean(itemToEdit.notes) || itemToEdit.restock_enabled === false;

        setSectionsOpen({
          basic: true,
          stock: true,
          purchase: true,
          packaging: hasCustomPackaging,
          consumption: hasConsumptionData,
          other: hasOtherData,
        });
      } else {
        setSectionsOpen({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formValid) {
      ...
      setSectionsOpen((prev) => ({
        ...prev,
        basic: true,
        packaging: form.consumable && !unitValid ? true : prev.packaging,
        purchase: purchaseErr ? true : prev.purchase,
      }));
      return;
    }
    ...
    if (
      ...
      (form.consumable &&
        (Number.isNaN(minStock) ||
          minStock < 0 ||
          Number.isNaN(unitsPerPackage) ||
          unitsPerPackage < 1))
    ) {
      setErr('Please check the numeric values in Stock or Packaging.');
      setSectionsOpen((prev) => ({ ...prev, stock: true, packaging: true }));
      return;
    }
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
            {/* Scrollable form body with clear sections */}
            <div
              ref={scrollBodyRef}
            ...
            >
              ...
              >
                ...
                <div className="block">
                  ...
                  {productInvalid && (
                    <p className="text-xs text-red-400 mt-1">Product name is required</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="Variant">
                    <input
                      ...
                      placeholder="e.g. Sensitive, Black"
                      enterKeyHint="next"
                      autoCapitalize="words"
                      className="input"
                    />
                  ...
                  {/* Specification */}
                  <Field label="Specification" hint="Descriptive">
                    <input
                      value={form.specification}
                      onChange={(e) => setForm({ ...form, specification: e.target.value })}
                      placeholder="e.g. 400 mL, AA"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`rounded-xl border p-2.5 text-left transition active:scale-[0.97] ${
                        form.consumable
                          ? 'border-emerald-500 bg-emerald-600/15 ring-1 ring-emerald-500/30'
                          : 'border-neutral-800 bg-neutral-800/40 hover:border-neutral-700'
                      }`}
                    >
                      <div
                        className={`text-sm font-medium ${
                          form.consumable ? 'text-emerald-300' : 'text-neutral-300'
                        }`}
                      >
                        Consumable
                      </div>
                      <div className="text-[11px] text-neutral-500 mt-0.5">
                        Gets used up (food, soap)
                      </div>
                    </button>
                    <button
                      type="button"
              ...
              <FormSection
                ...
                badge={
                  <span className="text-xs text-neutral-400 tabular-nums">
                    {form.count} {form.consumable ? pluralize(form.unit || 'piece', Number(form.count) || 0) : 'owned'}
                    {form.consumable && Number(form.min_stock) > 0 ? ` • Min ${form.min_stock}` : ''}
                  </span>
                }
              >
                {form.consumable ? (
              ...
              >
                ...
                <Field label="Price basis" hint="Unit price tracking">
                  <select
                    value={form.price_basis}
                    onChange={(e) =>
                      setForm({ ...form, price_basis: e.target.value as PriceBasis | '' })
                    }
                    className="input"
                  >
                    <option value="">None (Total price)</option>
              ...
              {/* 4. PACKAGING (Consumable Only) */}
              {form.consumable && (
                <FormSection
                  title="Packaging"
                  icon={Package}
                  open={sectionsOpen.packaging}
                  onToggle={() => toggleSection('packaging')}
                  badge={
                    <span className="text-xs text-neutral-400">
                      {form.unit || 'Piece'}
                      {form.purchase_package ? ` • ${form.purchase_package}` : ''}
                      {form.units_per_package !== '1' ? ` (${form.units_per_package}/pkg)` : ''}
                    </span>
                  }
                  error={unitInvalid ? 'Stock unit is required' : undefined}
                >
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      label="Stock unit"
                    ...
                    >
                      <input
                        ...
                        placeholder="e.g. bottle, roll, piece"
                    ...
                    <Field label="Purchase package" hint="Optional">
                      <input
                  ...
                  <p className="text-[11px] text-neutral-500">
                    {form.tracking_mode === 'packages'
                      ? 'Stock counts whole packages. Units per package is recorded for reference.'
                      : 'Stock counts individual units. Restocking packages multiplies by this quantity.'}
                  </p>
                </FormSection>
              )}

              {/* 5. CONSUMPTION (Consumable Only) */}
              {form.consumable && (
                <FormSection
                  title="Consumption"
                  icon={Clock}
                  open={sectionsOpen.consumption}
                ...
                >
                  <Field label="Opened on" hint="Date first opened / in use">
                    <div className="relative">
                      <input
                        type="date"
                        value={form.openedAt}
                  ...
                  {/* Consumption History Editor (if editing existing item) */}
                  {isEdit && itemToEdit ? (
                    <div className="pt-2">
                      <ConsumptionHistoryEditor inventoryId={itemToEdit.id} />
                    </div>
                  ) : (
                    <div className="rounded-xl bg-neutral-800/40 border border-neutral-800 p-3 flex items-start gap-2 text-xs text-neutral-400">
                      <Sparkles className="h-4 w-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span>
                        Opening duration and average consumption stats will track automatically as you use and restock this item.
                      </span>
                    </div>
                  )}
                </FormSection>
              ...
              <FormSection
                title="Other"
                icon={Sliders}
                open={sectionsOpen.other}
                onToggle={() => toggleSection('other')}
                badge={
                  form.notes.trim() ? (
                    <span className="text-xs text-neutral-400">Has notes</span>
                  ) : !form.restock_enabled && form.consumable ? (
                    <span className="text-xs text-neutral-500">Auto-restock off</span>
                  ) : undefined
                }
              >
                <Field label="Notes">
                ...
                {form.consumable && (
                  <div className="rounded-xl border border-neutral-800 bg-neutral-800/40 p-3.5 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((prev) => ({ ...prev, restock_enabled: !prev.restock_enabled }))
                      }
              ...
              {/* 7. DELETE PRODUCT (Secondary / destructive section at bottom of Edit screen) */}
              {isEdit && onDelete && itemToEdit && (
                <div className="pt-4 pb-2">
                  <div className="rounded-2xl border border-red-950/60 bg-red-950/20 p-4 space-y-2">
                    <div className="text-xs font-semibold uppercase tracking-wider text-red-400">
                      Delete Product
                    </div>
                    <p className="text-xs text-neutral-400">
                      Remove this product and its restock &amp; consumption history from your inventory.
                    </p>
                    <button
                      type="button"
                      onClick={() => setShowDeleteConfirm(true)}
                      disabled={submitting}
                      className="w-full h-11 px-4 rounded-xl bg-red-950/60 hover:bg-red-900/60 text-red-300 border border-red-800/60 font-medium text-sm active:scale-[0.98] transition flex items-center justify-center gap-2"
                    >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-[fadeIn_120ms_ease-out]">
          <div className="w-full max-w-xs bg-neutral-900 border border-neutral-800 rounded-2xl p-5 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-neutral-400 mb-5">
                This removes <span className="text-white font-medium">{itemToEdit.brand ? `${itemToEdit.brand} • ${itemToEdit.product}` : itemToEdit.product}</span> and all restock &amp; consumption history from your inventory.
              </p>
              <div className="w-full space-y-2">
                <button
                  type="button"
                  ...
                  disabled={submitting}
                  className="w-full h-11 px-4 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white font-medium text-sm active:scale-[0.98] transition"
                >
                  Cancel
                </button>
                <button
...
function FormSection({
  ...
  return (
    <div className={`rounded-2xl border transition-colors duration-150 ${error ? 'border-red-900/60 bg-red-950/10' : 'border-neutral-800/80 bg-neutral-800/30'}`}>
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between px-4 py-3 text-left transition rounded-2xl hover:bg-neutral-800/40"
      >
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 space-y-3 border-t border-neutral-800/50 animate-[fadeIn_120ms_ease-out]">
...
function ModeOption({
  ...
  return (
    ...
    >
      <div className={`text-sm font-medium ${active ? 'text-emerald-300' : 'text-neutral-200'}`}>
        {title}
      </div>
  ...
  return (
    <label className="block">
      <span className="flex items-baseline gap-1.5">
        ...
        {hint && !error && <span className="text-[10px] text-neutral-500 mb-1">{hint}</span>}
      </span>
      {children}
